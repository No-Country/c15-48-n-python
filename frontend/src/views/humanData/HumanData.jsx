...
export default function HumanData() {
  ...
  let humanUrl = `http://127.0.0.1:8000/api/user/sergiom/`; // tendria que ir al nombre no al username xq no tiene
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        console.log(ownerUser.first_name, ownerUser.email);
      })
      .catch((error) => {
  ...
  return (
    <div>
      ...
      <div className="mx-6 text-white font-custom mb-12">
        ...
        <p className="mb-1">{ownerUser != null ? ownerUser.first_name : ""}</p>
        <p className="mb-1">{ownerUser != null ? ownerUser.email : ""}</p>
      </div>
...
{/* {(ownerUser.pets !== null && ownerUser.pets.length !== 0) && (
  ownerUser.pets.map((pet) => (
  <div className="mx-5 font-custom text-white flex items-center" key={`${humanProfile.id}-${pet.id}`}>
    ...
    <div className="flex justify-between w-full items-center">
      ...
      <button className="bg-dark-gray rounded-full" onClick={handleDelete}>
        <img
